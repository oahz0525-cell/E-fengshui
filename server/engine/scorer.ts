import type { Element, Goal, Environment, XiShen, CalcResult, SubScores } from "@contracts/fengshui";
import { GOAL_DIR, EL_DIR } from "./data";
...
export function genEnv(lat: number, lng: number, floor = 1): Environment {
  const h = hash(`${lat.toFixed(5)},${lng.toFixed(5)},${new Date().getDate()}`);
  const h2 = hash(`${lng.toFixed(5)},${lat.toFixed(5)},${new Date().getDate() + 1}`);
  ...
  return {
    waterDist: 150 + h % 1000,
    parkDist: 80 + (h * 7) % 900,
    roadDist: 30 + (h * 3) % 500,
    metroDist: 50 + (h * 11) % 1200,
    cemeteryDist: 400 + (h * 13) % 3000,
...
export function calcResult(
  ...
  let score = 50;
  const subs: SubScores = { wealth: 50, creative: 50, sleep: 50, social: 50 };
  ...
  const subRecord = subs as unknown as Record<string, number>;
